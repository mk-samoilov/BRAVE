...
Реализован как встроенное поле `TransformField` на каждом Object. Хранит позицию и масштаб объекта в мировом пространстве.

```rust
obj.transform.set(x, y, z);              // задать позицию
obj.transform.get() -> Vec3;             // получить позицию

obj.transform.set_scale(x, y, z);        // задать масштаб по осям
obj.transform.get_scale() -> Vec3;       // получить масштаб
```

Transform создаётся автоматически при `spawn()` со значением позиции `(0.0, 0.0, 0.0)` и масштаба `(1.0, 1.0, 1.0)`.

Model matrix строится как `T * R * S` (translation × rotation × scale).
...
Оптимизация: мировая матрица пересчитывается **только при изменении** transform, rotate или scale (своего или родительского). При изменении устанавливается dirty flag. Перед рендером — обход дерева, пересчёт только помеченных, сброс флагов.
